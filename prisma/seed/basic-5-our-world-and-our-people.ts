// prisma/seed/basic-5-our-world-and-our-people.ts
...
// We use a loose type here so TS doesn't fight us about JSON shape
type JsonAny = any;
...
async function main() {
  ...
  const filePath = path.join(
    ...
    "seed",
    "curriculum",
    "basic-5-our-world-and-our-people.json"
  );

  const raw = fs.readFileSync(filePath, "utf8");
  const data: JsonAny = JSON.parse(raw);

  console.log("📖 Loading Basic 5 OWOP curriculum from:", filePath);
  ...
  // NOTE: We do NOT use `part` or `subject` here – those are not Prisma fields.
  ...
  // 3. Helper functions for nested data
  ...
  // IMPORTANT:
  // We use `strandId` / `subStrandId` and cast `data` as `any`
  // so TypeScript stops complaining, but Prisma still uses
  // the correct DB columns at runtime.
  ...
  // 4. Create strands (+ optional subStrands, contentStandards, indicators, exemplars)
  ...
  for (const strandData of data.strands as JsonAny[]) {
    ...
    // Case A: strand has subStrands (normal OWOP pattern)
  ...
  console.log("✅ Finished seeding Basic 5 Our World and Our People.");
}

main()
  .catch((e) => {
    console.error("❌ Error seeding Basic 5 Our World and Our People:", e);
    process.exit(1);
  })
  .finally(async () => {
