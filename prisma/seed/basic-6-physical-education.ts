...
/**
 * JSON shapes based on basic-6-physical-education.json
 */
...
async function main() {
  ...
  const filePath = path.join(
    ...
    'seed',
    'curriculum',
    'basic-6-physical-education.json'
  );

  console.log('📖 Loading Basic 6 Physical Education curriculum from:', filePath);
  ...
  console.log('✅ Seeded Basic 6 Physical Education with id:', subject.id);
  console.log('   Strands created from JSON:', data.strands?.length ?? 0);
}

main()
  .catch((e) => {
    console.error('❌ Error seeding Basic 6 Physical Education:', e);
    process.exitCode = 1;
  })
  .finally(async () => {
