// prisma/seed/basic-4-ghanaian-language.ts
...
async function main() {
  const filePath = path.join(
    ...
    'seed',
    'curriculum',
    'basic-4-ghanaian-language.json',
  );

  console.log(`📖 Loading Basic 4 Ghanaian Language from: ${filePath}`);

  const raw = fs.readFileSync(filePath, 'utf8');
  const json = JSON.parse(raw) as SubjectJson;

  if (!json || !json.slug || !Array.isArray(json.strands)) {
    throw new Error('Invalid JSON structure for Basic 4 Ghanaian Language');
  }
  ...
  console.log('✅ Finished seeding Basic 4 Ghanaian Language');
}

main()
  .catch((e) => {
    console.error('❌ Error seeding Basic 4 Ghanaian Language:', e);
    process.exit(1);
  })
  .finally(async () => {
