// prisma/seed/basic-6-english-language-part2.ts
...
async function upsertSubjectTree(data: SubjectJson) {
  const subject = await prisma.curriculumSubject.upsert({
    ...
    update: {
      ...
      phase: data.phase,
      level: data.level,
      // no `subject` relation field here – just simple scalar fields
    },
    create: {
      name: data.name,
      slug: data.slug,
...
async function main() {
  const filePath = path.join(
    ...
    'seed',
    'curriculum',
    'basic-6-english-language-part2.json',
  );

  console.log(
    `📖 Loading Basic 6 English Language (Part 2) from: ${filePath}`,
  );
  ...
  if (!json || !json.slug || !Array.isArray(json.strands)) {
    throw new Error(
      'Invalid JSON structure for Basic 6 English Language (Part 2)',
    );
  }
  ...
  console.log('✅ Finished seeding Basic 6 English Language (Part 2)');
}

main()
  .catch((e) => {
    console.error('❌ Error seeding Basic 6 English Language (Part 2):', e);
    process.exit(1);
  })
  .finally(async () => {
