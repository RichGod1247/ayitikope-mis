...
const FILE_NAME = "basic-4-religious-and-moral-education.json";
...
async function main() {
  ...
  console.log("📦 Seeding Basic 4 Religious and Moral Education from JSON in:", CURRICULUM_DIR);

  const fullPath = path.join(CURRICULUM_DIR, FILE_NAME);
  if (!fs.existsSync(fullPath)) {
    console.error("❌ Basic 4 R.M.E JSON file not found:", fullPath);
    process.exit(1);
  }

  console.log("📖 Loading Basic 4 R.M.E from:", fullPath);
  ...
  console.log("🎉 Done seeding Basic 4 R.M.E curriculum.");
  console.log({
    subjectCreated,
    subjectUpdated,
    strands,
...
main()
  .catch((e) => {
    console.error("❌ Error seeding Basic 4 R.M.E:", e);
    process.exitCode = 1;
  })
  .finally(async () => {
