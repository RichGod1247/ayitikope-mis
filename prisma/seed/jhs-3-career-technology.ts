...
/**
 * JSON shapes based on jhs-3-career-technology.json
 */
...
async function main() {
  const filePath = path.join(
    ...
    'seed',
    'curriculum',
    'jhs-3-career-technology.json'
  );

  console.log('📖 Loading JHS 3 Career Technology curriculum from:', filePath);
  ...
  console.log('✅ Seeded JHS 3 Career Technology with id:', subject.id);
  console.log('   Strands created from JSON:', data.strands?.length ?? 0);
}

main()
  .catch((e) => {
    console.error('❌ Error seeding JHS 3 Career Technology:', e);
    process.exitCode = 1;
  })
  .finally(async () => {
