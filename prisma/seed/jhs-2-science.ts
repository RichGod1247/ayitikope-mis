...
/**
 * Type shapes based on jhs-2-science.json
 */
...
async function main() {
  ...
  console.log('📖 Loading JHS 2 Science curriculum from:', filePath);
  ...
  console.log('✅ Seeded JHS 2 Science with id:', subject.id);
  console.log('   Strands created from JSON:', data.strands?.length ?? 0);
}

main()
  .catch((e) => {
    console.error('❌ Error seeding JHS 2 Science:', e);
    process.exitCode = 1;
  })
  .finally(async () => {
