// prisma/seed/basic-6-mathematics.ts
...
async function main() {
  const filePath = path.join(
    ...
    'seed',
    'curriculum',
    'basic-6-mathematics.json',
  );

  console.log(`📖 Loading Basic 6 Mathematics from: ${filePath}`);

  const raw = fs.readFileSync(filePath, 'utf8');
  const json = JSON.parse(raw) as SubjectJson;

  if (!json || !json.slug || !Array.isArray(json.strands)) {
    throw new Error('Invalid JSON structure for Basic 6 Mathematics');
  }
  ...
  console.log('✅ Finished seeding Basic 6 Mathematics');
}

main()
  .catch((e) => {
    console.error('❌ Error seeding Basic 6 Mathematics:', e);
    process.exit(1);
  })
  .finally(async () => {
