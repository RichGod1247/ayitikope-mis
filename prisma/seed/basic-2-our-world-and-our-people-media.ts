...
function logDbTarget() {
  ...
  try {
    const u = new URL(raw);
    // username in pooler URLs often includes project ref; safe to show
...
async function mainReal() {
  ...
  const seedPath = path.join(
    __dirname,
    "curriculum",
    "basic-2-our-world-and-our-people-media.json"
  );

  console.log("📖 Loading B2 OWOP media seed from:", seedPath);
  ...
  console.log("🎉 Done seeding B2 OWOP media.");
...
async function main() {
  try {
    ...
    console.error("❌ Error in B2 OWOP media seed:", err);
  } finally {
