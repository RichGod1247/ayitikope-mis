// prisma/seed/basic-4-history.ts
// Seed script for Basic 4 History curriculum
...
const SUBJECT_FRIENDLY_NAME = 'Basic 4 History';
const SUBJECT_JSON_FILE = 'basic-4-history.json';

// -------- MODEL ALIASES (same pattern as the working scripts) --------
...
// Optional: strip inline [cite: ###] markers from descriptions if they appear
function cleanText(s: string | null | undefined): string | null {
  if (!s) return s ?? null;
  return s.replace(/\[cite:[^\]]*]/g, '').replace(/\s+/g, ' ').trim();
}
          ...
          for (const exJson of indicatorJson.exemplars ?? []) {
            await exemplarModel.create({
              data: {
                ...
                description: cleanText(exJson.description),
              },
            });
          }
        }
...
async function main() {
  try {
    ...
    console.error('❌ Error seeding Basic 4 History:', err);
    process.exitCode = 1;
  } finally {
