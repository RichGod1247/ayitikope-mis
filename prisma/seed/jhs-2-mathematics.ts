...
/**
 * JSON type shapes based on jhs-2-mathematics.json
 */
...
async function main() {
  const filePath = path.join(
    ...
    'seed',
    'curriculum',
    'jhs-2-mathematics.json'
  );

  console.log('📖 Loading JHS 2 Mathematics curriculum from:', filePath);
  ...
  console.log('✅ Seeded JHS 2 Mathematics with id:', subject.id);
  console.log('   Strands created from JSON:', data.strands?.length ?? 0);
}

main()
  .catch((e) => {
    console.error('❌ Error seeding JHS 2 Mathematics:', e);
    process.exitCode = 1;
  })
  .finally(async () => {
