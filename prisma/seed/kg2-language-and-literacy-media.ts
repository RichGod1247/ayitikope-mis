//prisma/seed/kg2-language-and-literacy-media.ts
...
function resolveSeedPath(): string {
  const candidates = [
    path.join(__dirname, "curriculum", "kg2-language-and-literacy-media.clean.json"),
    path.join(__dirname, "curriculum", "kg2-language-and-literacy-media.json"),
  ];
...
function validateDataset(items: MediaSeedRow[]) {
  if (!Array.isArray(items) || items.length === 0) {
    throw new Error("No rows found in KG2 Language and Literacy media JSON.");
  }
...
async function mainReal() {
  ...
  console.log("📖 Loading KG2 Language and Literacy media seed from:", seedPath);
  ...
  if (!subject) {
    throw new Error(
      `CurriculumSubject not found for slug='${subjectSlug}'. Seed KG2 Language and Literacy curriculum first.`
    );
  }
  ...
  console.log("🎉 Done seeding KG2 Language and Literacy media.");
...
async function main() {
  try {
    ...
    console.error("❌ Error in KG2 Language and Literacy media seed:", err);
    process.exit(1);
  } finally {
