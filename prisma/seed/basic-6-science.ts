// prisma/seed/basic-6-science.ts
import fs from 'fs';
...
const SUBJECT_FRIENDLY_NAME = 'Basic 6 Science';
const SUBJECT_JSON_FILE = 'basic-6-science.json';
    ...
    for (const sub of strand.subStrands ?? []) {
      ...
      for (const cs of sub.contentStandards ?? []) {
        ...
        for (const indicator of cs.indicators ?? []) {
          ...
          // 6) Exemplars – wipe & recreate for each indicator
