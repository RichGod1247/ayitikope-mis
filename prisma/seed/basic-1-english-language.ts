...
async function main() {
  ...
  console.log(
    `📖 Loading Basic 1 English Language curriculum from: ${jsonPath}`
  );
  ...
  const created = await prisma.curriculumSubject.create({
    data: {
      ...
      // No baseSubject for now – same pattern as Maths.
      strands: {
  ...
  console.log(
    `✅ Seeded Basic 1 English Language with id: ${created.id}`
  );
  console.log(
...
main()
  .catch((err) => {
    console.error("❌ Error seeding Basic 1 English Language:", err);
    process.exit(1);
  })
  .finally(async () => {
