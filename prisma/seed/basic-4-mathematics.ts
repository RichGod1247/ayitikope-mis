// prisma/seed/basic-4-mathematics.ts
...
async function main() {
  const filePath = path.join(
    ...
    'seed',
    'curriculum',
    'basic-4-mathematics.json',
  );

  console.log(`📖 Loading Basic 4 Mathematics from: ${filePath}`);

  const raw = fs.readFileSync(filePath, 'utf8');
  const json = JSON.parse(raw) as SubjectJson;

  if (!json || !json.slug || !Array.isArray(json.strands)) {
    throw new Error('Invalid JSON structure for Basic 4 Mathematics');
  }
  ...
  console.log('✅ Finished seeding Basic 4 Mathematics');
}

main()
  .catch((e) => {
    console.error('❌ Error seeding Basic 4 Mathematics:', e);
    process.exit(1);
  })
  .finally(async () => {
