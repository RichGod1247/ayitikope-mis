// prisma/seed/kg2-creative-arts-media.ts
...
function resolveSeedPath(): string {
  const candidates = [
    path.join(__dirname, "curriculum", "kg2-creative-arts-media.clean.json"),
    path.join(__dirname, "curriculum", "kg2-creative-arts-media.json"),
  ];
...
function validateDataset(items: MediaSeedRow[]) {
  if (!Array.isArray(items) || items.length === 0) {
    throw new Error("No rows found in KG2 Creative Arts media JSON.");
  }
...
async function mainReal() {
  ...
  console.log("📖 Loading KG2 Creative Arts media seed from:", seedPath);
  ...
  if (!subject) {
    throw new Error(
      `CurriculumSubject not found for slug='${subjectSlug}'. Seed KG2 Creative Arts curriculum first.`
    );
  }
  ...
  console.log("🎉 Done seeding KG2 Creative Arts media.");
...
async function main() {
  try {
    ...
    console.error("❌ Error in KG2 Creative Arts media seed:", err);
    process.exit(1);
  } finally {
