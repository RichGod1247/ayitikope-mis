...
// JSON file for KG2 Creative Arts media
const MEDIA_JSON_PATH = path.join(
  __dirname,
  'curriculum',
  'kg2-creative-arts-media.json'
);

// This must match the slug in edulife_os."CurriculumSubject"
const SUBJECT_SLUG = 'kg2-creative-arts';
...
// Make sure imagePath always looks like: /curriculum/kg2/creative-arts/...
...
async function main() {
  console.log(`📖 Loading KG2 Creative Arts media seed from: ${MEDIA_JSON_PATH}`);
  ...
  console.log(
    `\n🎉 Done seeding KG2 Creative Arts media. ` +
...
main()
  .catch((e) => {
    console.error('❌ Error in KG2 Creative Arts media seed:', e);
    process.exit(1);
  })
  .finally(async () => {
