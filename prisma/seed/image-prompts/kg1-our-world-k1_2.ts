// prisma/seed/image-prompts/kg1-our-world-k1_2.ts
...
async function main() {
  const filePath = path.join(__dirname, "kg1-our-world-k1_2.json");
  const raw = fs.readFileSync(filePath, "utf8");
  const data: FileShape = JSON.parse(raw);

  if (!data.imagePrompts || data.imagePrompts.length === 0) {
    console.warn("⚠️ No imagePrompts found in kg1-our-world-k1_2.json");
    return;
  }

  console.log(
    `Seeding ${data.imagePrompts.length} image prompts for ${data.subject} (${data.level}) – strand ${data.imagePrompts[0]?.strandCode}`,
  );
  ...
  for (const item of data.imagePrompts) {
    ...
    const strandCode = item.strandCode; // always present in your JSON
  ...
  console.log("✅ Done seeding image prompts for KG1 – Our World and Our People, Strand K1.2.");
}

main()
  .catch((e) => {
    console.error("❌ Error seeding image prompts for K1.2", e);
    process.exit(1);
  })
  .finally(async () => {
