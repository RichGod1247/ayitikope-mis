// prisma/seed/image-prompts/kg1-our-world-k1_3.ts
...
async function main() {
  const filePath = path.join(__dirname, "kg1-our-world-k1_3.json");
  const raw = fs.readFileSync(filePath, "utf8");
  const data: FileShape = JSON.parse(raw);

  if (!data.imagePrompts || data.imagePrompts.length === 0) {
    console.warn("⚠️ No imagePrompts found in kg1-our-world-k1_3.json");
    return;
  }
  ...
  console.log("✅ Done seeding image prompts for KG1 – Our World and Our People, Strand K1.3.");
}

main()
  .catch((e) => {
    console.error("❌ Error seeding image prompts for K1.3", e);
    process.exit(1);
  })
  .finally(async () => {
