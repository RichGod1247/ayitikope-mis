// prisma/seed/merge-basic-2-our-world-and-our-people.ts
...
type IndicatorJson = {
  ...
  // imagePrompts and any other extra fields are allowed at runtime
};
...
const PART_FILES = [
  "basic-2-our-world-and-our-people-part-1.json",
  "basic-2-our-world-and-our-people-part-2.json",
];
...
async function main() {
  console.log("🔄 Merging Basic 2 Our World and Our People parts...");

  const parts = await Promise.all(PART_FILES.map(loadPart));
  const [first, ...rest] = parts;

  // sanity check to make sure all parts belong to the same subject
  for (const p of rest) {
  ...
  const outPath = path.join(
    __dirname,
    "curriculum",
    "basic-2-our-world-and-our-people.json"
  );
  ...
  console.log(
    `✅ Merged Basic 2 OWOP → ${outPath}\n   Total strands: ${mergedStrands.length}`
  );
}

main().catch((err) => {
  console.error("❌ Error merging Basic 2 Our World and Our People:", err);
  process.exit(1);
});
