// prisma/seed/basic-6-english-language-part1.ts
...
async function upsertSubjectTree(data: SubjectJson) {
  const subject = await prisma.curriculumSubject.upsert({
    ...
    update: {
      ...
      phase: data.phase,
      level: data.level,
      // NOTE: no `subject` relation field in the Prisma model
    },
    create: {
      name: data.name,
      slug: data.slug,
...
async function main() {
  const filePath = path.join(
    ...
    'seed',
    'curriculum',
    'basic-6-english-language-part1.json',
  );

  console.log(
    `📖 Loading Basic 6 English Language (Part 1) from: ${filePath}`,
  );
  ...
  if (!json || !json.slug || !Array.isArray(json.strands)) {
    throw new Error(
      'Invalid JSON structure for Basic 6 English Language (Part 1)',
    );
  }
  ...
  console.log('✅ Finished seeding Basic 6 English Language (Part 1)');
}

main()
  .catch((e) => {
    console.error('❌ Error seeding Basic 6 English Language (Part 1):', e);
    process.exit(1);
  })
  .finally(async () => {
