...
/**
 * JSON type shapes based on jhs-3-mathematics.json
 */
...
async function main() {
  const filePath = path.join(
    ...
    'seed',
    'curriculum',
    'jhs-3-mathematics.json'
  );

  console.log('📖 Loading JHS 3 Mathematics curriculum from:', filePath);
  ...
  console.log('✅ Seeded JHS 3 Mathematics with id:', subject.id);
  console.log('   Strands created from JSON:', data.strands?.length ?? 0);
}

main()
  .catch((e) => {
    console.error('❌ Error seeding JHS 3 Mathematics:', e);
    process.exitCode = 1;
  })
  .finally(async () => {
