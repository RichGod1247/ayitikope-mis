...
const OUT_JSON = path.join(
  process.cwd(),
  "prisma/seed/curriculum/basic-6-creative-arts-media.json"
);

function categoryFromCode(code: string): "visual-arts" | "performing-arts" {
  // DB uses: "B6 1.x.x.x" for Visual, "B6 2.x.x.x" for Performing
  if (code.startsWith("B6 1.")) return "visual-arts";
  if (code.startsWith("B6 2.")) return "performing-arts";
  return "visual-arts";
}

function filenameFromCode(code: string): string {
  // DB code "B6 1.1.1.1" -> file "B6.1.1.1.png"
  const dotBase = code.replace(" ", ".");
  return `${dotBase}.png`; // CANONICAL. NO ".1" EVER.
}

function imagePathFromCode(code: string): string {
  const cat = categoryFromCode(code);
  const file = filenameFromCode(code);
  return `https://pub-f33886c26f33473d91e2bf1505b9df29.r2.dev/upper-primary/basic-6/basic-6-creative-arts/${cat}/${file}`;
}

function altFrom(code: string, desc: string): string {
  return `Basic 6 Creative Arts ${code}: ${desc}`.slice(0, 180);
}
...
async function main() {
  ...
  const rows = indicators.map((i) => ({
    indicatorId: i.id,
    indicatorCode: i.code, // DB source-of-truth (with spaces)
    indicatorDescription: i.description,
    pageNumberInPdf: 0,
    figureLabel: null,
    imagePath: imagePathFromCode(i.code),
    altText: altFrom(i.code, i.description || "Illustration."),
    detailedDescription: `Illustration for Basic 6 Creative Arts indicator ${i.code} — ${i.description}`,
    tags: `curriculum,upper-primary,basic-6,creative-arts,${categoryFromCode(i.code)}`,
  }));
