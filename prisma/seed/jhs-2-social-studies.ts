...
/**
 * Type shapes based on jhs-2-social-studies.json
 */
...
async function main() {
  const filePath = path.join(
    ...
    'seed',
    'curriculum',
    'jhs-2-social-studies.json'
  );

  console.log('📖 Loading JHS 2 Social Studies curriculum from:', filePath);
  ...
  console.log('✅ Seeded JHS 2 Social Studies with id:', subject.id);
  console.log('   Strands created from JSON:', data.strands?.length ?? 0);
}

main()
  .catch((e) => {
    console.error('❌ Error seeding JHS 2 Social Studies:', e);
    process.exitCode = 1;
  })
  .finally(async () => {
