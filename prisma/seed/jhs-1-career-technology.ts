...
/**
 * JSON shapes based on jhs-1-career-technology.json
 */
...
// Ensure non-empty description for required DB fields
...
async function main() {
  const filePath = path.join(
    ...
    'seed',
    'curriculum',
    'jhs-1-career-technology.json'
  );

  console.log('📖 Loading JHS 1 Career Technology curriculum from:', filePath);
  ...
  // 1️⃣ Clear any existing copy for this slug (idempotent seeding)
  try {
  ...
  console.log('✅ Seeded JHS 1 Career Technology with id:', subject.id);
  console.log('   Strands created from JSON:', data.strands?.length ?? 0);
}

main()
  .catch((e) => {
    console.error('❌ Error seeding JHS 1 Career Technology:', e);
    process.exitCode = 1;
  })
  .finally(async () => {
