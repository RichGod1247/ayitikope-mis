...
/**
 * Type shapes based on jhs-1-religious-and-moral-education.json
 */
...
async function main() {
  const filePath = path.join(
    ...
    'seed',
    'curriculum',
    'jhs-1-religious-and-moral-education.json'
  );

  console.log('📖 Loading JHS 1 RME curriculum from:', filePath);
  ...
  console.log('✅ Seeded JHS 1 Religious and Moral Education with id:', subject.id);
  console.log('   Strands created from JSON:', data.strands?.length ?? 0);
}

main()
  .catch((e) => {
    console.error('❌ Error seeding JHS 1 Religious and Moral Education:', e);
    process.exitCode = 1;
  })
  .finally(async () => {
