...
async function mainReal() {
  const seedPath = path.join(
    __dirname,
    'curriculum',
    'kg2-language-and-literacy-media.json'
  );

  console.log(
    '📖 Loading KG2 Language & Literacy media seed from:',
    seedPath
  );
  ...
  console.log('🎉 Done seeding KG2 Language & Literacy media.');
}
...
async function main() {
  try {
    ...
    console.error('❌ Error in KG2 Language & Literacy media seed:', err);
  } finally {
