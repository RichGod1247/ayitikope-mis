...
/**
 * JSON shapes based on basic-4-physical-education.json
 */
...
async function main() {
  ...
  const filePath = path.join(
    ...
    'seed',
    'curriculum',
    'basic-4-physical-education.json'
  );

  console.log('📖 Loading Basic 4 Physical Education curriculum from:', filePath);
  ...
  console.log('✅ Seeded Basic 4 Physical Education with id:', subject.id);
  console.log('   Strands created from JSON:', data.strands?.length ?? 0);
}

main()
  .catch((e) => {
    console.error('❌ Error seeding Basic 4 Physical Education:', e);
    process.exitCode = 1;
  })
  .finally(async () => {
