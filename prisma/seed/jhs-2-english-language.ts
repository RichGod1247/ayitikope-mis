...
/**
 * JSON type shapes based on jhs-2-english-language.json
 */
...
// Ensure non-empty description for DB required fields
...
async function main() {
  const filePath = path.join(
    ...
    'seed',
    'curriculum',
    'jhs-2-english-language.json'
  );

  console.log('📖 Loading JHS 2 English Language curriculum from:', filePath);
  ...
  console.log('✅ Seeded JHS 2 English Language with id:', subject.id);
  console.log('   Strands created from JSON:', data.strands?.length ?? 0);
}

main()
  .catch((e) => {
    console.error('❌ Error seeding JHS 2 English Language:', e);
    process.exitCode = 1;
  })
  .finally(async () => {
