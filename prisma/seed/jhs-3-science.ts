...
/**
 * Type shapes based on jhs-3-science.json
 */
...
async function main() {
  ...
  console.log('📖 Loading JHS 3 Science curriculum from:', filePath);
  ...
  console.log('✅ Seeded JHS 3 Science with id:', subject.id);
  console.log('   Strands created from JSON:', data.strands?.length ?? 0);
}

main()
  .catch((e) => {
    console.error('❌ Error seeding JHS 3 Science:', e);
    process.exitCode = 1;
  })
  .finally(async () => {
