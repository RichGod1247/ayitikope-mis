// prisma/seed/basic-6-creative-arts.ts
// Seed script for Basic 6 Creative Arts curriculum (JSON part 1/4)
...
const SUBJECT_FRIENDLY_NAME = 'Basic 6 Creative Arts';
// NOTE: Currently we only have part 1/4 JSON, so we point directly to part-1.
// If later you add part-2/3/4, change this to "basic-6-creative-arts-part-"
// so it picks up all of them.
const SUBJECT_PART_PREFIX = 'basic-6-creative-arts-part-1';
const SUBJECT_SLUG = 'basic-6-creative-arts';

/**
 * MODEL ALIASES
 *
 * - phase & level models are OPTIONAL (we just log and skip if they don’t exist).
 * - CurriculumSubject exists and has fields: id, name, slug, description, orderIndex, phase, level, etc.
 * - Child models (strand, subStrand, contentStandard, indicator, exemplar) are required.
 */
const models = {
  phase:
...
async function loadSubjectsFromParts(): Promise<SubjectJson[]> {
  const curriculumDir = path.join(__dirname, 'curriculum');
  console.log(
    `📦 Seeding ${SUBJECT_FRIENDLY_NAME} from JSON parts in: ${curriculumDir}`,
  );

  const allFiles = await fs.readdir(curriculumDir);
  const files = allFiles
    .filter(
      (f) =>
        f.startsWith(SUBJECT_PART_PREFIX) &&
        f.toLowerCase().endsWith('.json'),
    )
    .map((f) => path.join(curriculumDir, f));

  if (!files.length) {
    throw new Error(
      `No JSON part files found for ${SUBJECT_FRIENDLY_NAME} with prefix "${SUBJECT_PART_PREFIX}" in ${curriculumDir}`,
    );
  }

  console.log(
    `🔍 Found ${SUBJECT_FRIENDLY_NAME} part files: ${JSON.stringify(
  ...
  console.log(
    `✅ Loaded ${subjects.length} subject(s) for ${SUBJECT_FRIENDLY_NAME}`,
  );
  return subjects;
}
...
async function upsertSubjectTree(subjectJson: SubjectJson) {
  ...
  // SUBJECT – CurriculumSubject does have slug/phase/level
...
async function main() {
  try {
    const subjects = await loadSubjectsFromParts();
    ...
    console.error('❌ Error seeding Basic 6 Creative Arts:', err);
    process.exitCode = 1;
  } finally {
