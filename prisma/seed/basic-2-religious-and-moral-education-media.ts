...
async function mainReal() {
  ...
  const seedPath = path.join(
    __dirname,
    "curriculum",
    "basic-2-religious-and-moral-education-media.json"
  );

  console.log("📖 Loading B2 Religious and Moral Education media seed from:", seedPath);
  ...
  for (const row of items) {
    ...
    if (!indicator) {
      ...
      console.log("   👉 Means the curriculum seed is missing that indicator OR codes differ.");
      console.log();
      skipped++;
      continue;
    }
  ...
  console.log("🎉 Done seeding B2 Religious and Moral Education media.");
...
async function main() {
  try {
    ...
    console.error("❌ Error in B2 Religious and Moral Education media seed:", err);
  } finally {
