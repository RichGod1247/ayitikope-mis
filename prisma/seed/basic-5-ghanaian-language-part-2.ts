// prisma/seed/basic-5-ghanaian-language-part-2.ts
...
async function upsertSubjectTree(data: SubjectJson) {
  ...
  console.log(`   Strands in JSON (part 2): ${data.strands.length}`);
...
async function main() {
  const filePath = path.join(
    ...
    'seed',
    'curriculum',
    'basic-5-ghanaian-language-part-2.json',
  );

  console.log(`📖 Loading Basic 5 Ghanaian Language (Part 2) from: ${filePath}`);

  const raw = fs.readFileSync(filePath, 'utf8');
  const json = JSON.parse(raw) as SubjectJson;

  if (!json || !json.slug || !Array.isArray(json.strands)) {
    throw new Error('Invalid JSON structure for Basic 5 Ghanaian Language (Part 2)');
  }
  ...
  console.log('✅ Finished seeding Basic 5 Ghanaian Language (Part 2)');
}

main()
  .catch((e) => {
    console.error('❌ Error seeding Basic 5 Ghanaian Language (Part 2):', e);
    process.exit(1);
  })
  .finally(async () => {
