...
/**
 * JSON type shapes based on jhs-1-english-language.json
 */
...
async function main() {
  const filePath = path.join(
    ...
    'seed',
    'curriculum',
    'jhs-1-english-language.json'
  );

  console.log('📖 Loading JHS 1 English Language curriculum from:', filePath);
  ...
  console.log('✅ Seeded JHS 1 English Language with id:', subject.id);
  console.log('   Strands created from JSON:', data.strands?.length ?? 0);
}

main()
  .catch((e) => {
    console.error('❌ Error seeding JHS 1 English Language:', e);
    process.exitCode = 1;
  })
  .finally(async () => {
