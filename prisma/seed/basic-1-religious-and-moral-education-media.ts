...
function logDbTarget() {
  ...
  try {
    ...
    console.log(`ðŸ”Œ DB target: ${host}:${port} | user: ${user}`);
  } catch {
    console.log("ðŸ”Œ DB target: (could not parse DATABASE_URL)");
  }
}
...
async function mainReal() {
  ...
  const seedPath = path.join(
    __dirname,
    "curriculum",
    "basic-1-religious-and-moral-education-media.json"
  );

  console.log("ðŸ“– Loading B1 Religious and Moral Education media seed from:", seedPath);
  ...
  for (const row of items) {
    console.log(`â†’ Processing indicator ${row.indicatorCode} (${row.subjectSlug})`);
    ...
    if (!indicator) {
      ...
      if (indicator) {
        fallbackHits++;
        console.log("   ðŸ§­ Used fallback lookup (code + subjectSlug).");
      }
    }

    if (!indicator) {
      console.log(
        `   âš ï¸ Could not find indicator ${row.indicatorCode} in DB for subjectSlug=${row.subjectSlug}.`
      );
      console.log(
        "   ðŸ‘‰ This means the curriculum seed is missing that indicator, OR the code differs in the curriculum JSON."
      );
      console.log();
      skipped++;
      continue;
    ...
    try {
      ...
      if (existing) {
        ...
        console.log(`   âœ… Created (id=${newRow.id})\n`);
        created++;
      }
    } catch (e) {
      console.log("   âŒ Failed to seed this row:", e);
      console.log();
      failed++;
    }
  }

  console.log("ðŸŽ‰ Done seeding B1 Religious and Moral Education media.");
...
async function main() {
  try {
    ...
    console.error("âŒ Error in B1 Religious and Moral Education media seed:", err);
  } finally {
