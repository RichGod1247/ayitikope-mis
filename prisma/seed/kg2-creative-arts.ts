...
async function main() {
  ...
  const jsonPath = path.join(__dirname, "curriculum", "kg2-creative-arts.json");

  const raw = await fs.readFile(jsonPath, "utf8");
  const curriculum = JSON.parse(raw) as CurriculumSubjectJson;

  console.log(`📖 Loading KG2 Creative Arts curriculum from: ${jsonPath}`);
  ...
  console.log(`✅ Seeded KG2 Creative Arts with id: ${created.id}`);
