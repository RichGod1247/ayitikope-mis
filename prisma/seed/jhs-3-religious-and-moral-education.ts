...
/**
 * Type shapes based on jhs-3-religious-and-moral-education.json
 */
...
async function main() {
  const filePath = path.join(
    ...
    'seed',
    'curriculum',
    'jhs-3-religious-and-moral-education.json'
  );

  console.log('📖 Loading JHS 3 RME curriculum from:', filePath);
  ...
  console.log('✅ Seeded JHS 3 Religious and Moral Education with id:', subject.id);
  console.log('   Strands created from JSON:', data.strands?.length ?? 0);
}

main()
  .catch((e) => {
    console.error('❌ Error seeding JHS 3 Religious and Moral Education:', e);
    process.exitCode = 1;
  })
  .finally(async () => {
