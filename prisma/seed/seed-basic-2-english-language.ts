...
// Types for the final merged JSON (no "part" field here)
type ExemplarJson = {
  orderIndex: number;
  description: string;
};
...
async function main() {
  ...
  console.log(
    `📖 Loading Basic 2 English Language curriculum from: ${jsonPath}`
  );
  ...
  const created = await prisma.curriculumSubject.create({
    data: {
      ...
      // No baseSubject for now – same pattern as Maths/Basic 1 English.
      strands: {
  ...
  console.log(
    `✅ Seeded Basic 2 English Language with id: ${created.id}`
  );
  console.log(
...
main()
  .catch((err) => {
    console.error("❌ Error seeding Basic 2 English Language:", err);
    process.exit(1);
  })
  .finally(async () => {
