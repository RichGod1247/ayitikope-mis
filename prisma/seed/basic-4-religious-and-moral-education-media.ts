...
async function mainReal() {
  ...
  const seedPath = path.join(
    __dirname,
    "curriculum",
    "basic-4-religious-and-moral-education-media.json"
  );

  console.log("📖 Loading B4 Religious and Moral Education media seed from:", seedPath);

  if (!fs.existsSync(seedPath)) {
    console.error("❌ Media JSON not found:", seedPath);
    process.exit(1);
  }
  ...
  for (const row of items) {
    ...
    if (!indicator) {
      ...
      console.log(
        "   👉 This means the curriculum seed is missing that indicator, OR the code differs in the curriculum JSON."
      );
      console.log();
      skipped++;
      continue;
    ...
    try {
      ...
      if (existing) {
        await prisma.curriculumMedia.update({
          ...
          data: {
            figureLabel: row.figureLabel ?? undefined,
            imagePath: row.imagePath,
            altText: row.altText,
  ...
  console.log("🎉 Done seeding B4 Religious and Moral Education media.");
...
async function main() {
  try {
    ...
    console.error("❌ Error in B4 Religious and Moral Education media seed:", err);
    process.exitCode = 1;
  } finally {
