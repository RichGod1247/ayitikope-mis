// prisma/seed/basic-1-creative-arts.ts
...
async function main() {
  ...
  console.log(
    `📖 Loading Basic 1 Creative Arts curriculum from: ${jsonPath}`
  );
  ...
  // If this subject already exists, delete it so we always reseed cleanly
  ...
  console.log(
    `✅ Seeded Basic 1 Creative Arts with id: ${created.id}`
  );
  console.log(
...
main()
  .catch((err) => {
    console.error("❌ Error seeding Basic 1 Creative Arts:", err);
    process.exit(1);
  })
  .finally(async () => {
