...
/**
 * Type shapes based on jhs-1-social-studies.json
 */
...
async function main() {
  const filePath = path.join(
    ...
    'seed',
    'curriculum',
    'jhs-1-social-studies.json'
  );

  console.log('📖 Loading JHS 1 Social Studies curriculum from:', filePath);
  ...
  console.log('✅ Seeded JHS 1 Social Studies with id:', subject.id);
  console.log('   Strands created from JSON:', data.strands?.length ?? 0);
}

main()
  .catch((e) => {
    console.error('❌ Error seeding JHS 1 Social Studies:', e);
    process.exitCode = 1;
  })
  .finally(async () => {
