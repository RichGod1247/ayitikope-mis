...
async function main() {
  const seedPath = path.join(
    __dirname,
    "curriculum",
    "b3-creative-arts-performing-media.json"
  );

  console.log(
    `📖 Loading B3 Creative Arts (Performing) media seed from: ${seedPath}`
  );
  ...
  console.log("\n🎉 Done seeding B3 Creative Arts (Performing) media.");
}
...
async function mainReal() {
  try {
    ...
    console.error("❌ Error in B3 Creative Arts (Performing) media seed:", err);
  } finally {
