// prisma/seed/basic-5-ghanaian-language-part-1.ts
...
async function upsertSubjectTree(data: SubjectJson) {
  ...
  console.log(`   Strands in JSON (part 1): ${data.strands.length}`);
    ...
    for (const sub of strand.subStrands) {
      ...
      for (const cs of sub.contentStandards) {
        ...
        for (const ind of cs.indicators) {
          ...
          // EXEMPLARS: clear & recreate for this indicator
...
async function main() {
  const filePath = path.join(
    ...
    'seed',
    'curriculum',
    'basic-5-ghanaian-language-part-1.json',
  );

  console.log(`📖 Loading Basic 5 Ghanaian Language (Part 1) from: ${filePath}`);

  const raw = fs.readFileSync(filePath, 'utf8');
  const json = JSON.parse(raw) as SubjectJson;

  if (!json || !json.slug || !Array.isArray(json.strands)) {
    throw new Error('Invalid JSON structure for Basic 5 Ghanaian Language (Part 1)');
  }
  ...
  console.log('✅ Finished seeding Basic 5 Ghanaian Language (Part 1)');
}

main()
  .catch((e) => {
    console.error('❌ Error seeding Basic 5 Ghanaian Language (Part 1):', e);
    process.exit(1);
  })
  .finally(async () => {
