// prisma/seed/basic-6-ghanaian-language.ts
...
async function main() {
  const filePath = path.join(
    ...
    'seed',
    'curriculum',
    'basic-6-ghanaian-language.json',
  );

  console.log(`📖 Loading Basic 6 Ghanaian Language from: ${filePath}`);

  const raw = fs.readFileSync(filePath, 'utf8');
  const json = JSON.parse(raw) as SubjectJson;

  if (!json || !json.slug || !Array.isArray(json.strands)) {
    throw new Error('Invalid JSON structure for Basic 6 Ghanaian Language');
  }
  ...
  console.log('✅ Finished seeding Basic 6 Ghanaian Language');
}

main()
  .catch((e) => {
    console.error('❌ Error seeding Basic 6 Ghanaian Language:', e);
    process.exit(1);
  })
  .finally(async () => {
