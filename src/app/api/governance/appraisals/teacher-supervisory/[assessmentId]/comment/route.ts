...
import { saveTeacherSupervisoryGeneralComment } from "@/lib/appraisals/teacherSupervisoryAssessmentScoring";
import {
  clean,
  isUuidIdentifier,
  jsonNoStore,
...
export async function POST(req: NextRequest, context: RouteContext) {
  ...
  if (
    ...
    body.notApplicable != null
  ) {
    return jsonNoStore(400, {
      ok: false,
      reqId: meta.reqId,
      error: "TEACHER_SUPERVISORY_COMMENT_ENDPOINT_COMMENT_ONLY",
    });
  }

  try {
    const result = await saveTeacherSupervisoryGeneralComment({
    ...
    return teacherSupervisoryApiError({
      error,
      reqId: meta.reqId,
      logTag: "[TEACHER_SUPERVISORY_COMMENT_API_ERROR]",
    });
  }
}
