//src/app/api/governance/notices/attachments/verify/route.ts
...
import {
  ...
  verifyGovernanceNoticeAttachmentUpload,
...
export async function POST(req: NextRequest) {
  ...
  try {
    const result = await verifyGovernanceNoticeAttachmentUpload({
      scope: auth.scope,
    ...
    console.error("[GOVERNANCE_NOTICE_ATTACHMENT_VERIFY_ERROR]", {
      reqId,
      error,
    });
    ...
    return json(500, {
      ok: false,
      reqId,
      error: "FAILED_TO_VERIFY_NOTICE_ATTACHMENT",
    });
  }
}
