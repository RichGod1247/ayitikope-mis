//src/app/api/governance/notices/attachments/inspect/route.ts
...
import {
  inspectGovernanceNoticeAttachment,
} from "@/lib/governance/noticeAttachmentInspection";
...
export async function POST(req: NextRequest) {
  ...
  try {
    const result = await inspectGovernanceNoticeAttachment({
      scope: auth.scope,
    ...
    console.error("[GOVERNANCE_NOTICE_ATTACHMENT_INSPECT_ERROR]", {
      reqId,
      error,
    });
    ...
    return json(500, {
      ok: false,
      reqId,
      error: "FAILED_TO_INSPECT_NOTICE_ATTACHMENT",
    });
  }
}
