//src/app/api/governance/notices/recipients/preview/route.ts
...
import {
  ...
  previewGovernanceNoticeRecipients,
...
export async function POST(req: NextRequest) {
  ...
  try {
    const result = await previewGovernanceNoticeRecipients({
      scope: auth.scope,
      input: body,
    });
    ...
    console.error("[GOVERNANCE_NOTICE_RECIPIENT_PREVIEW_ERROR]", {
      reqId,
      error,
    });
    ...
    return json(500, {
      ok: false,
      reqId,
      error: "FAILED_TO_PREVIEW_NOTICE_RECIPIENTS",
    });
  }
}
