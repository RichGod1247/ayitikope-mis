...
export async function POST(req: Request) {
  ...
  // Pre-fetch context needed for receipt + SMS (read-only, outside transaction)
  ...
  try {
    const result = await prisma.$transaction(async (tx) => {
      ...
      if (currentBalance <= 0) return { ok: false as const, status: 400, error: "INVOICE_ALREADY_CLEARED" };
      if (amountPesewas > currentBalance) return { ok: false as const, status: 400, error: "PAYMENT_EXCEEDS_BALANCE" };
      ...
      const outstandingPesewas = Math.max(0, currentBalance - amountPesewas);
