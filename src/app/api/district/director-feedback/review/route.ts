// src/app/api/district/director-feedback/review/route.ts
import { randomUUID } from "crypto";
import { NextRequest, NextResponse } from "next/server";
import {
  getDirectorFeedbackReviewWorkspace,
  startDirectorFeedbackReview,
...
function safeError(error: unknown, reqId: string) {
  ...
  const code = /^[A-Z0-9_]{3,140}$/.test(candidate)
    ? candidate
    : "FAILED_TO_MANAGE_DIRECTOR_FEEDBACK_REVIEW";

  if (status >= 500) {
    console.error("[DISTRICT_DIRECTOR_FEEDBACK_REVIEW_ERROR]", {
      reqId,
      error,
    });
  }
...
export async function POST(req: NextRequest) {
  ...
  if (!body || body.confirm !== true) {
    return jsonNoStore(400, {
      ok: false,
      reqId,
      error: "DIRECTOR_FEEDBACK_REVIEW_CONFIRMATION_REQUIRED",
    });
  }
  ...
  try {
    const result = await startDirectorFeedbackReview({
      actorUserId: auth.ctx.userId,
      actorRoleName: auth.ctx.roleName,
      cycleId,
      reqId,
