...
import { extendExpiredDirectorFeedbackCycle } from "@/lib/appraisals/directorFeedbackDeadlineExtension";
...
function safeError(error: unknown, reqId: string) {
  ...
  if (status >= 500) {
    console.error("[DISTRICT_DIRECTOR_FEEDBACK_EXTENSION_ERROR]", {
      reqId,
      error,
    });
  }
...
export async function POST(req: NextRequest, context: RouteContext) {
  ...
  if (Number.isFinite(contentLength) && contentLength > MAX_BODY_BYTES) {
    return jsonNoStore(413, {
      ok: false,
      reqId,
      error: "DIRECTOR_FEEDBACK_EXTENSION_BODY_TOO_LARGE",
    });
  }
  ...
  if (!bodyContainsOnlyAllowedFields(body as Record<string, unknown>)) {
    return jsonNoStore(400, {
      ok: false,
      reqId,
      error: "DIRECTOR_FEEDBACK_EXTENSION_FIELDS_FORBIDDEN",
    });
  }
  ...
  if (body.confirm !== true) {
    return jsonNoStore(400, {
      ok: false,
      reqId,
      error: "DIRECTOR_FEEDBACK_EXTENSION_CONFIRMATION_REQUIRED",
    });
  }

  try {
    const result = await extendExpiredDirectorFeedbackCycle({
