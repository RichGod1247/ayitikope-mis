...
export async function POST(req: Request) {
  try {
    ...
    // ✅ REQUIRE bootstrap invite token
    ...
    if (!tenantName) return NextResponse.json({ ok: false, error: "School name is required." }, { status: 400 });
    if (!email || !email.includes("@")) return NextResponse.json({ ok: false, error: "Valid email is required." }, { status: 400 });
    if (!password || password.length < 8) {
      return NextResponse.json({ ok: false, error: "Password must be at least 8 characters." }, { status: 400 });
    }

    // ✅ Bind enrollment to invite contact email (prevents forwarding)
    ...
    // Slug preference (invite reserved wins)
    ...
    const result = await prisma.$transaction(
      async (tx) => {
        ...
        if (existing) throw Object.assign(new Error("Email already exists."), { code: "EMAIL_EXISTS" });

        // Create tenant (PENDING until approved)
        ...
        return { tenantId: tenant.id, userId: user.id, slug: tenant.slug, schoolCode: tenant.schoolCode, status: tenant.status };
      },
    ...
    if (err?.code === "EMAIL_EXISTS") return NextResponse.json({ ok: false, error: "Email already exists." }, { status: 409 });
    if (String(err?.code) === "P2002") return NextResponse.json({ ok: false, error: "Duplicate value. Try again." }, { status: 409 });
