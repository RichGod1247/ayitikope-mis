...
type StudentItem = {
  ...
  lastName: string;
  sex: string | null;
  dob: string | null; // ISO string from server (we’ll convert safely)
...
export default function StudentsClient({ items, tenantSlug, classrooms }: Props) {
  ...
  async function handleCreate(e: React.FormEvent) {
    ...
    const payload: any = {
      ...
      sex: cForm.sex || null,
      guardianName: cForm.guardianName || null,
      guardianPhone: cForm.guardianPhone || null,
      classroomId: cForm.classroomId || null,
    };
    if (cForm.dob) {
      // convert yyyy-mm-dd to ISO midnight UTC
      payload.dob = new Date(cForm.dob + "T00:00:00Z").toISOString();
    } else {
      payload.dob = null;
    }
    ...
    try {
      ...
      setList((prev) => [data.item as StudentItem, ...prev]);
  ...
  async function handleUpdate(id: string) {
    const payload: any = {
      ...
      sex: eForm.sex || null,
      guardianName: eForm.guardianName || null,
      guardianPhone: eForm.guardianPhone || null,
      classroomId: eForm.classroomId || null,
    };
    payload.dob = eForm.dob ? new Date(eForm.dob + "T00:00:00Z").toISOString() : null;
    ...
    try {
      ...
      // Align with server’s returned item (in case of triggers/defaults)
      setList((cur) => {
        const i = cur.findIndex((x) => x.id === id);
        if (i < 0) return cur;
        const clone = [...cur];
        clone[i] = data.item as StudentItem;
        return clone;
      });
      closeEdit();
    } catch (err: any) {
    ...
    <div className="space-y-8">
      <div className="rounded-xl border p-5">
        <h2 className="text-lg font-semibold mb-3">
          Add Student {tenantSlug ? <span className="text-gray-400">({tenantSlug})</span> : null}
        </h2>
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                        <div className="space-y-1">
                          ...
                          <input
                            ...
                            onChange={(e) =>
                              setEForm((f) => ({ ...f, firstName: e.target.value }))
                            }
                          />
                        </div>
                        <div className="space-y-1">
                          ...
                          <input
                            ...
                            onChange={(e) =>
                              setEForm((f) => ({ ...f, lastName: e.target.value }))
                            }
                          />
                        </div>
                        ...
                        <div className="space-y-1">
                          ...
                          <select
                            ...
                            value={eForm.sex}
                            onChange={(e) =>
                              setEForm((f) => ({ ...f, sex: e.target.value }))
                            }
                          >
                            <option value="">--</option>
                            <option value="M">Male (M)</option>
                        ...
                        <div className="space-y-1">
                          ...
                          <input
                            ...
                            value={eForm.dob}
                            onChange={(e) =>
                              setEForm((f) => ({ ...f, dob: e.target.value }))
                            }
                          />
                        </div>
