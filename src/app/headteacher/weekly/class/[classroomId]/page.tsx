...
export default function WeeklyClassRosterPage() {
  ...
  async function load() {
    ...
    setMsg(null)
    try {
      const url = `/api/headteacher/weekly/class-roster?tenantId=${encodeURIComponent(
        tenantId
      )}&classroomId=${encodeURIComponent(classroomId)}&start=${encodeURIComponent(start)}&end=${encodeURIComponent(end)}`
  ...
  return (
    <div className="p-6 max-w-6xl mx-auto space-y-6">
      ...
      <div className="overflow-x-auto">
        <table className="min-w-full border rounded-xl overflow-hidden">
          ...
          <tbody>
            ...
            {!rows.length && (
              <tr>
                <td className="p-4 text-sm text-zinc-600" colSpan={11}>
      ...
      <div className="flex gap-3">
        <a className={btnOutline} href={`/headteacher/weekly/class/${encodeURIComponent(classroomId)}?start=${encodeURIComponent(start)}&end=${encodeURIComponent(end)}`}>
          ← Back to Class Days
        </a>
