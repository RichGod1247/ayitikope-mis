...
export default function HeadteacherDayPage() {
  ...
  const [loadingRow, setLoadingRow] = useState<string | null>(null) // sessionId while certifying/reopening

  function todayYYYYMMDD() { const now = new Date(); return now.toISOString().slice(0, 10) }

  // Week range Mon–Fri for the date (or today)
  ...
  async function refresh(opts?: { clearMsg?: boolean }) {
    ...
    try {
      const res = await fetch(`/api/headteacher/day/overview?tenantId=${encodeURIComponent(tenantId)}&date=${encodeURIComponent(d)}`)
  ...
  async function certifyOne(sessionId: string) {
    setMsg('Certifying…'); setLoadingRow(sessionId)
    try {
      ...
      if (!res.ok) { setMsg(`Certify failed: ${data?.error || res.status}`); return }
      setMsg('Certified.')
  ...
  async function reopenOne(sessionId: string) {
    setMsg('Reopening…'); setLoadingRow(sessionId)
    try {
      ...
      const data = await res.json().catch(() => ({}))
      if (!res.ok) { setMsg(`Reopen failed: ${data?.error || res.status}`); return }
  ...
  async function bulkCertify() {
    if (!tenantId) return
    const d = (date || '').trim() || todayYYYYMMDD()
    setMsg('Bulk certifying…'); setLoadingBulk(true)
    try {
      ...
      const data = await res.json().catch(() => ({}))
      if (!res.ok) { setMsg(`Bulk certify failed: ${data?.error || res.status}`); return }
  ...
  const weeklyCsvURL = tenantId
    ? `/api/headteacher/export-csv-range?tenantId=${encodeURIComponent(tenantId)}&start=${encodeURIComponent(start)}&end=${encodeURIComponent(end)}`
    : ''

  return (
    <div className="p-6 max-w-5xl mx-auto space-y-6">
      <h1 className="text-2xl font-bold">Headteacher Day View</h1>
      <p className="text-sm text-zinc-600">Pick a date, see each class status, certify individually or in bulk, export (Mon–Fri), or reopen to correct mistakes.</p>
      ...
      <div className="flex flex-wrap gap-3 items-end">
        ...
        <div className="flex-1 min-w-60">
          ...
          <div className="flex gap-2">
            ...
            />
            <button className={btnOutline + ' shrink-0'} onClick={() => setDate(todayYYYYMMDD())}>Today</button>
            <button className={btnGhost + ' shrink-0'} onClick={() => setDate('')}>Clear</button>
          </div>
          <div className="text-xs text-zinc-600 mt-1">Week (Mon–Fri): {start} → {end}</div>
        </div>
        ...
        <div className="flex flex-wrap gap-3">
          <button
            ...
            onClick={() => refresh({ clearMsg: true })}
            disabled={!tenantId || loadingPage}
          >
          ...
          <a
            className={`${btnOutline} ${weeklyCsvURL ? '' : 'pointer-events-none opacity-50'}`}
            href={weeklyCsvURL}
          >
          ...
          <div key={it.classroomId} className="flex items-center justify-between border rounded-xl p-3">
            ...
            <div className="flex items-center gap-2">
              <a
                ...
                href={it.sessionId ? `/api/attendance/sessions/export-csv?tenantId=${encodeURIComponent(tenantId)}&classroomId=${encodeURIComponent(it.classroomId)}&date=${encodeURIComponent((date||'').trim() || todayYYYYMMDD())}` : '#'}
              >
                CSV
              </a>
              ...
              <button
                ...
                title={!it.sessionId ? 'No session to certify' : (it.status === 'CERTIFIED' ? 'Already certified' : 'Certify this class')}
              >
              ...
              <button
                ...
                title={!it.sessionId ? 'No session to reopen' : 'Reopen to make corrections'}
              >
