...
export default function PendingTenantsClient() {
  ...
  async function load() {
    ...
    setMsg(null);
    try {
      const r = await fetch("/api/admin/super/tenants/pending/list", { cache: "no-store" });
      ...
      {loading ? (
        ...
        <div className="space-y-2">
          {items.map((t) => {
            ...
            return (
              <div key={t.id} className="border rounded-xl p-3 space-y-3">
                <div className="flex items-start justify-between gap-3">
                  ...
                  <div className="shrink-0 flex items-center gap-2">
                    <button
                      ...
                      className="h-9 px-3 rounded-xl border border-zinc-300 bg-black text-white hover:bg-zinc-800 text-sm"
                    >
                      Approve
                    </button>
                    ...
                    <button
                      ...
                      className="h-9 px-3 rounded-xl border border-rose-200 bg-rose-50 text-rose-700 hover:bg-rose-100 text-sm"
                    >
                      Reject
                    </button>
                  </div>
                ...
                {rejectingId === t.id ? (
                  <div className="rounded-xl border border-zinc-200 bg-zinc-50 p-3 space-y-2">
                    <div className="text-xs text-zinc-700 font-semibold">Reject reason (required)</div>
                    <input
