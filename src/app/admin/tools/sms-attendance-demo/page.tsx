...
export default function SmsAttendanceDemoPage() {
  ...
  const parsedStudents = useMemo(() => parseStudents(rawStudents), [rawStudents]);
  ...
  return (
    <main className="min-h-screen flex justify-center bg-slate-50 py-8 px-4">
      <div className="w-full max-w-3xl bg-white shadow-md rounded-xl p-6 border border-slate-200">
        <h1 className="text-2xl font-bold mb-2">EduLife OS – Attendance SMS Demo</h1>
        <p className="text-sm text-slate-600 mb-4">
          Prototype tool for sending <strong>attendance alerts</strong> to parents/guardians.
          In this demo version, you manually specify student names and phone numbers. Later,
          this will plug directly into the attendance records.
        </p>
        ...
        <form onSubmit={handleSend} className="space-y-4">
          ...
          <div>
            ...
            <p className="mt-1 text-xs text-slate-500">
              Example:{" "}
              <code className="bg-slate-100 px-1 py-0.5 rounded">John Doe - 0241234567</code>.
          ...
          <div>
            ...
            <p className="mt-1 text-xs text-slate-500">
              This is the template that will be sent for each student, with their name inserted.
            </p>
          </div>
        ...
        {result && (
          <div className="mt-6">
            <h2 className="text-sm font-semibold text-slate-800 mb-2">Result</h2>
            ...
            {!result.ok ? (
              ...
              <div className="rounded-lg bg-emerald-50 border border-emerald-200 px-3 py-2 text-sm text-emerald-800">
                ...
                <strong>
                  brand = {result.brand} • class = {result.className} • date = {result.date}
                </strong>
                . Success: <strong>{result.successCount}/{result.count}</strong>
              </div>
            )}
                ...
                <table className="min-w-full text-xs">
                  ...
                  <tbody>
                    {result.results.map((r, i) => (
                      ...
                      >
                        <td className="px-3 py-2 text-slate-700">{r.student}</td>
                        ...
                        <td className="px-3 py-2 text-slate-700">
                          ...
                          {r.error && (
                            <div className="mt-0.5 text-[10px] text-slate-500">{r.error}</div>
                          )}
                        </td>
                      </tr>
                    ))}
        ...
        <div className="mt-4 text-xs text-slate-500">
          <p>
            All these messages are also logged into <code>SmsLog</code> and visible at{" "}
            <code>/admin/tools/sms-logs</code>. When we connect this to the real
            attendance system, this page can evolve into a “review & send” console instead
            of manual entry.
          </p>
        </div>
      </div>
