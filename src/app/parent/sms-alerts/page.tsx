...
const ParentSmsAlertsPage: React.FC = () => {
  ...
  useEffect(() => {
    async function load() {
      // Only load when we have the two key params
      ...
      try {
        ...
        const res = await fetch(
          `/api/parent/sms/history?${params.toString()}`
        );
        ...
        try {
          ...
          console.error(
            "[ParentSmsAlertsPage] Failed to parse JSON:",
            text
          );
          setFetchState("error");
          setErrorMessage(
        ...
        if (!res.ok || !("ok" in json) || json.ok === false) {
          ...
          console.error(
            "[ParentSmsAlertsPage] HTTP error:",
            res.status,
            msg
          );
          setFetchState("error");
        ...
        console.error(
          "[ParentSmsAlertsPage] Error loading SMS history",
          err
        );
        setFetchState("error");
        setErrorMessage(
          "Something went wrong while loading your SMS alerts. Please try again."
        );
      }
    }
  ...
  return (
    <main className="min-h-screen bg-slate-50">
      <div className="mx-auto max-w-3xl px-4 py-6 sm:py-8">
        ...
        <header className="mb-6 flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
          <div>
            ...
            <p className="mt-1 text-sm text-slate-600">
              A simple list of recent SMS messages sent to your phone
              from the school. This helps you track fee reminders,
        ...
        {/* If params missing, show guidance */}
        {!hasParams && (
          <section className="rounded-xl border border-dashed border-slate-300 bg-slate-50 px-4 py-5 text-xs sm:text-sm text-slate-600">
            <p>
              To view your SMS alerts, open this page with your{" "}
              <span className="font-semibold">tenantId</span> and{" "}
              <span className="font-semibold">guardianPhone</span> in
              the URL, for example:
            </p>
...
{`/parent/sms-alerts?tenantId=cmhhnghn00008vcpgp3fl07fl
  &guardianPhone=0240000000`}
            ...
            <p className="mt-2 text-[11px] text-slate-500">
              Later, this page will be opened automatically from your
              parent dashboard, so you won&apos;t need to copy links
              yourself.
            </p>
          </section>
        )}
          ...
          <section className="mt-4 space-y-4">
            {!hasRecords ? (
              <div className="rounded-xl border border-slate-200 bg-white px-4 py-5 text-xs sm:text-sm text-slate-600">
                No SMS alerts found for this phone yet. Once the
                school starts sending fee reminders or health
              ...
              <div className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
                ...
                <p className="mt-1 text-xs text-slate-600">
                  The most recent messages appear at the top.
                </p>
...
export default ParentSmsAlertsPage;
