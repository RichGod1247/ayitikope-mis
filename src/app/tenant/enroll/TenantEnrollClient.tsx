...
export default function TenantEnrollClient() {
  ...
  useEffect(() => {
    async function run() {
      ...
      try {
        const r = await fetch(`/api/tenant-bootstrap/lookup?token=${encodeURIComponent(token)}`, {
          cache: "no-store",
        });
  ...
  async function submit() {
    ...
    try {
      ...
      if (!r.ok || !j || j.ok !== true) {
        const msg =
          typeof j === "object" && j
            ? String((j as { message?: unknown; error?: unknown }).message ?? (j as { error?: unknown }).error ?? `Failed (${r.status}).`)
            : `Failed (${r.status}).`;
        setErr(msg);
        return;
      }
