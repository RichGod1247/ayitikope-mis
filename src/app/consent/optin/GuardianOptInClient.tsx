// src/app/consent/optin/GuardianOptInClient.tsx
"use client";
...
export default function GuardianOptInClient({
  tenantId,
  studentId,
}: {
  tenantId: string;
  ...
  // ✅ Public page: load ONLY the target student via detail endpoint.
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        const r = await fetch(
          `/api/consent/students/detail?tenantId=${encodeURIComponent(
            tenantId
        ...
        if (!r.ok || j?.ok === false) {
          setStudent(null);
          setError(j?.error || "Failed to load student.");
          return;
        }

        const s = j?.student || null;
        if (!s) {
          setStudent(null);
          setError("Student not found for this link.");
          return;
        }
        ...
        setStudent({
          ...
          smsOptIn: Boolean(s.smsOptIn),
        });
      } catch (e: any) {
        if (!ac.signal.aborted) setError(e?.message || "Failed to load student");
      } finally {
        if (!ac.signal.aborted) setLoadingStudent(false);
      }
    })();
  ...
  async function handleOptIn() {
    ...
    try {
      ...
      if (!r.ok || !j?.ok) throw new Error(j?.error || "Failed to save consent");
  ...
  async function handleDecline() {
    ...
    try {
      ...
      const j = await r.json().catch(() => ({}));
      if (!r.ok || !j?.ok) throw new Error(j?.error || "Failed to record decline");
    ...
    <div className="min-h-screen bg-white">
      <div className="border-b">
        <div className="max-w-3xl mx-auto px-4 py-4 flex items-center gap-3">
          ...
          />
          <div>
            <div className="text-lg font-semibold">EduLife OS</div>
            <div className="text-xs text-zinc-600">
              Guardian Consent & SMS Preferences
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="max-w-3xl mx-auto p-4 space-y-5">
        {!tenantId || !studentId ? (
          ...
          <div className="rounded-xl border border-red-200 bg-red-50 p-4 text-sm text-red-700">
            {error || "Student not found. Please contact the school office."}
          </div>
        ) : (
          <>
                ...
                <div className="text-right">
                  ...
                  <div className="text-sm">
                    {student.healthConsentAt
                      ? new Date(student.healthConsentAt).toLocaleString()
                      : "—"}
                  </div>
