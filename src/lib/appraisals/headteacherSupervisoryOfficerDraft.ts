...
export const HEADTEACHER_SUPERVISORY_OFFICER_DRAFT_POLICY = {
  schemaVersion: 1,
...
export type HeadteacherSupervisoryOfficerRole =
  (typeof HEADTEACHER_SUPERVISORY_OFFICER_DRAFT_POLICY.allowedActorRoles)[number];

const OFFICER_CYCLE_CREATED_AUDIT_ACTION =
  "HEADTEACHER_SUPERVISORY_OFFICER_GOVERNANCE_CARRIER_CREATED";
const OFFICER_DRAFT_CREATED_AUDIT_ACTION =
  "HEADTEACHER_SUPERVISORY_OFFICER_GOVERNANCE_DRAFT_CREATED";
const RELEASES_METADATA_KEY = "headteacherSupervisoryReleases";
const REVIEWED_RELEASE_MODE = "DIRECTOR_REVIEWED_GOVERNANCE_RELEASE";

export type CreateHeadteacherSupervisoryOfficerDraftInput = {
  actorUserId: string;
  ...
  now?: Date;
  database?: HeadteacherSupervisoryOfficerDraftDatabase;
};

export type HeadteacherSupervisoryOfficerDraftSummary = {
  cycleId: string;
  cycleStatus: "CLOSED";
  assessmentId: string;
  ...
  assessorRole: HeadteacherSupervisoryOfficerRole;
...
export type CreateHeadteacherSupervisoryOfficerDraftResult = {
  outcome: "CREATED" | "EXISTING_MATCH";
  draft: HeadteacherSupervisoryOfficerDraftSummary;
};
...
type ExistingOfficerCycleRecord = CycleRecord & {
...
type ExistingActorAssessmentRecord = {
  id: string;
  status: string;
  revision: number;
  cycle: {
...
type AppraisalCycleDelegate = {
  ...
  findMany(args: unknown): Promise<ExistingOfficerCycleRecord[]>;
...
type AppraisalAssessmentDelegate = {
  ...
  findMany(args: unknown): Promise<ExistingActorAssessmentRecord[]>;
  create(args: unknown): Promise<AssessmentRecord>;
};

export type HeadteacherSupervisoryOfficerDraftTransactionClient = {
  membership: {
...
export type HeadteacherSupervisoryOfficerDraftDatabase = {
  appraisalCycle: AppraisalCycleDelegate;
  appraisalAssessment: AppraisalAssessmentDelegate;
  $transaction<T>(
    operation: (
      tx: HeadteacherSupervisoryOfficerDraftTransactionClient,
    ) => Promise<T>,
    options?: {
...
type VisitContextSnapshot = {
  ...
  assessor: {
    userId: string;
    name: string | null;
    role: HeadteacherSupervisoryOfficerRole;
    assignmentId: string;
    assignmentRole: HeadteacherSupervisoryOfficerRole;
    scopeLevel: "DISTRICT" | "CIRCUIT";
  };
  jurisdiction: {
    districtZoneId: string;
...
function requireIdentifier(value: unknown, fieldName: string) {
  ...
  if (!/^[A-Za-z0-9_-]{5,180}$/.test(id)) {
    fail("HEADTEACHER_SUPERVISORY_OFFICER_DRAFT_INVALID_IDENTIFIER", 400, {
      fieldName,
    });
  }
  return id;
...
function requireDirectAssessmentKey(value: unknown) {
  ...
  if (
    ...
    fail("HEADTEACHER_SUPERVISORY_OFFICER_DRAFT_KEY_INVALID", 400, {
...
function requireObservationDate(value: Date | string, now: Date) {
  const raw = value instanceof Date ? isoDateOnly(value) : clean(value);
...
function cycleIdempotencyKey(input: {
  ...
  const digest = createHash("sha256")
    .update(
      [
        HEADTEACHER_SUPERVISORY_OFFICER_DRAFT_POLICY.carrierKind,
    ...
    .digest("hex");
  return `headteacher-supervisory-officer:${digest}`;
}
...
function officerRole(value: unknown): HeadteacherSupervisoryOfficerRole {
  const role = canonicalHeadteacherSupervisoryAssessorRole(value);
  if (
    !HEADTEACHER_SUPERVISORY_OFFICER_DRAFT_POLICY.allowedActorRoles.includes(
      role as HeadteacherSupervisoryOfficerRole,
    )
  ) {
    fail("HEADTEACHER_SUPERVISORY_OFFICER_DRAFT_ROLE_FORBIDDEN", 403, {
      role,
    });
  }
  return role as HeadteacherSupervisoryOfficerRole;
}
...
function assertGovernanceScope(input: {
  ...
  const tenantIds = new Set(input.scope.tenantIds.map(clean).filter(Boolean));
  if (!tenantIds.has(input.targetTenantId)) {
    fail("HEADTEACHER_SUPERVISORY_OFFICER_DRAFT_TENANT_OUT_OF_SCOPE", 403);
  }
  ...
  if (
    ...
    fail("HEADTEACHER_SUPERVISORY_OFFICER_DRAFT_ZONE_OUT_OF_SCOPE", 403);
  }
}
...
function targetSnapshot(
  ...
  if (
    ...
    fail("HEADTEACHER_SUPERVISORY_OFFICER_DRAFT_TARGET_CONTEXT_INVALID", 409);
  }
...
function findAssignment(records: AssignmentRecord[], assignmentId: string) {
  ...
  if (matches.length !== 1) {
    fail(
      "HEADTEACHER_SUPERVISORY_OFFICER_DRAFT_ASSIGNMENT_SNAPSHOT_INVALID",
      409,
  ...
  if (
    ...
    fail("HEADTEACHER_SUPERVISORY_OFFICER_DRAFT_ASSIGNMENT_ZONE_INACTIVE", 409);
  }
  return assignment;
}
...
function buildVisitContext(input: {
  ...
  actorRole: HeadteacherSupervisoryOfficerRole;
  scopeLevel: "DISTRICT" | "CIRCUIT";
  ...
  return {
    schemaVersion:
      HEADTEACHER_SUPERVISORY_OFFICER_DRAFT_POLICY.visitContextSchemaVersion,
    ...
    assessor: {
      ...
      assignmentRole: input.actorRole,
...
function reviewedGovernanceReleaseRecorded(input: {
  cycleMetadata: unknown;
  assessmentId: string;
  assessmentStatus: string;
}) {
  const releases = objectValue(
    objectValue(input.cycleMetadata)[RELEASES_METADATA_KEY],
  );
  const release = objectValue(releases[input.assessmentId]);
  return (
    normalized(input.assessmentStatus) === "FINALIZED" &&
    clean(release.releaseMode) === REVIEWED_RELEASE_MODE &&
...
function releasedAssessmentInCycle(
  cycle: ExistingOfficerCycleRecord,
  assessment: ExistingOfficerCycleRecord["assessments"][number],
) {
  return reviewedGovernanceReleaseRecorded({
...
function isOfficerGovernanceCarrier(input: {
  cycle: ExistingOfficerCycleRecord;
  actorUserId: string;
  actorRole: HeadteacherSupervisoryOfficerRole;
}) {
  const metadata = objectValue(input.cycle.metadata);
  const instrument = input.cycle.instrumentVersion.instrument;
  return (
    clean(metadata.workflow) === HEADTEACHER_SUPERVISORY_ASSESSMENT_POLICY.workflow &&
    clean(metadata.evidenceStream) === "GOVERNANCE_SUPERVISORY_ASSESSMENT" &&
    clean(metadata.carrierKind) ===
      HEADTEACHER_SUPERVISORY_OFFICER_DRAFT_POLICY.carrierKind &&
    clean(metadata.assessorUserId) === input.actorUserId &&
    canonicalHeadteacherSupervisoryAssessorRole(metadata.assessorRole) ===
      input.actorRole &&
    input.cycle.requestedByUserId === input.actorUserId &&
    input.cycle.instrumentVersion.version ===
      HEADTEACHER_SUPERVISORY_ASSESSMENT_POLICY.instrumentVersion &&
    normalized(input.cycle.instrumentVersion.status) === "ACTIVE" &&
...
function assertNoUnresolvedActorAssessment(
  assessments: ExistingActorAssessmentRecord[],
) {
  for (const assessment of assessments) {
    if (
      reviewedGovernanceReleaseRecorded({
        cycleMetadata: assessment.cycle.metadata,
        assessmentId: assessment.id,
        assessmentStatus: assessment.status,
      })
    ) {
    ...
    if (
      ["DRAFT", "RETURNED", "FINALIZED"].includes(
        normalized(assessment.status),
      )
    ) {
      fail("HEADTEACHER_SUPERVISORY_OFFICER_DRAFT_EXISTING_ACTIVE", 409, {
        cycleId: assessment.cycle.id,
        assessmentId: assessment.id,
        status: normalized(assessment.status),
        reason: "CONTINUE_CORRECT_OR_RELEASE_EXISTING_GOVERNANCE_ASSESSMENT",
      });
    }
  }
}

function assertNoOtherUnfinishedOfficerAssessment(input: {
  cycles: ExistingOfficerCycleRecord[];
  actorUserId: string;
  actorRole: HeadteacherSupervisoryOfficerRole;
}) {
  for (const cycle of input.cycles) {
    if (
      !isOfficerGovernanceCarrier({
        cycle,
    ...
    if (!assessment) {
      fail("HEADTEACHER_SUPERVISORY_OFFICER_DRAFT_ATOMIC_CYCLE_INCOMPLETE", 409, {
        cycleId: cycle.id,
      });
    }

    if (releasedAssessmentInCycle(cycle, assessment)) continue;
...
function assertExistingCycleContext(input: {
  ...
  if (
    ...
    input.cycle.identityVisibility !==
      HEADTEACHER_SUPERVISORY_OFFICER_DRAFT_POLICY.identityVisibility ||
    input.cycle.responseWindowDays !== 0 ||
    input.cycle.minimumResponses !== 0 ||
    ...
    normalized(metadata.scopeLevel) !== input.scopeLevel ||
    metadata.respondentWorkflow !== false ||
    clean(metadata.participantSelection) !== "NONE" ||
    ...
    fail("HEADTEACHER_SUPERVISORY_OFFICER_DRAFT_CONTEXT_DRIFT", 409, {
...
function buildDraftSummary(input: {
  ...
  expectedActorRole: HeadteacherSupervisoryOfficerRole;
  expectedScopeLevel: "DISTRICT" | "CIRCUIT";
  expectedContext: VisitContextSnapshot;
  expectedContextHash: string;
  keyHash: string;
  targetTenantId: string;
}): HeadteacherSupervisoryOfficerDraftSummary {
  ...
  if (
    ...
    input.assessment.revision !==
      HEADTEACHER_SUPERVISORY_OFFICER_DRAFT_POLICY.initialRevision ||
    ...
    clean(metadata.carrierKind) !==
      HEADTEACHER_SUPERVISORY_OFFICER_DRAFT_POLICY.carrierKind ||
    clean(metadata.assessorUserId) !== input.assessment.assessorUserId ||
    canonicalHeadteacherSupervisoryAssessorRole(metadata.assessorRole) !==
      input.expectedActorRole ||
    normalized(metadata.scopeLevel) !== input.expectedScopeLevel ||
  ...
  if (
    ...
    fail("HEADTEACHER_SUPERVISORY_OFFICER_DRAFT_PREMATURE_EVIDENCE", 409, {
      assessmentId: input.assessment.id,
    });
  }
  ...
  return {
    ...
    assessorRole: input.expectedActorRole,
    scopeLevel: input.expectedScopeLevel,
...
async function performDraftTransaction(input: {
  ...
  return input.database.$transaction(
    async (
      tx: HeadteacherSupervisoryOfficerDraftTransactionClient,
    ): Promise<CreateHeadteacherSupervisoryOfficerDraftResult> => {
      ...
      if (memberships.length === 0) {
        fail("HEADTEACHER_SUPERVISORY_OFFICER_DRAFT_TARGET_NOT_FOUND", 404);
      }
      if (memberships.length !== 1) {
        fail("HEADTEACHER_SUPERVISORY_OFFICER_DRAFT_TARGET_AMBIGUOUS", 409, {
      ...
      if (!actor) {
        fail("HEADTEACHER_SUPERVISORY_OFFICER_DRAFT_ASSESSOR_NOT_FOUND", 404);
      }
      ...
      if (!authority.allowed) {
        fail(
          `HEADTEACHER_SUPERVISORY_OFFICER_DRAFT_AUTHORITY_${authority.reason}`,
          403,
          { reason: authority.reason },
        );
      }
      if (authority.effectiveRole !== input.actorRole) {
        fail("HEADTEACHER_SUPERVISORY_OFFICER_DRAFT_ROLE_DRIFT", 409);
      }
      if (
        (input.actorRole === "SISSO" && authority.scopeLevel !== "CIRCUIT") ||
        (input.actorRole !== "SISSO" && authority.scopeLevel !== "DISTRICT")
      ) {
        fail("HEADTEACHER_SUPERVISORY_OFFICER_DRAFT_SCOPE_LEVEL_DRIFT", 409);
      }
      ...
      if (existingCycle) {
        ...
        const context = buildVisitContext({
          ...
          closedAt: existingCycle.closedAt!,
          target,
          actor,
          actorRole: input.actorRole,
          scopeLevel: authority.scopeLevel,
        ...
        return {
          ...
          draft: buildDraftSummary({
            ...
            expectedActorRole: input.actorRole,
            expectedScopeLevel: authority.scopeLevel,
      ...
      const existingActorAssessments = await tx.appraisalAssessment.findMany({
        where: {
          assessorUserId: actor.id,
      ...
      assertNoUnresolvedActorAssessment(existingActorAssessments);

      const existingOfficerCycles = await tx.appraisalCycle.findMany({
        where: {
          targetUserId: target.target.userId,
          targetTenantId: target.target.tenantId,
          targetRoleSnapshot: "HEADTEACHER",
      ...
      assertNoOtherUnfinishedOfficerAssessment({
        cycles: existingOfficerCycles,
        actorUserId: actor.id,
        actorRole: input.actorRole,
      });
      ...
      if (
        normalized(cycle.status) !== "CLOSED" ||
        !cycle.openedAt ||
        !cycle.closedAt ||
        ...
        fail(
          "HEADTEACHER_SUPERVISORY_OFFICER_DRAFT_ATOMIC_CYCLE_CONTRACT_INVALID",
          409,
          { cycleId: cycle.id },
        );
      }
      ...
      const assessment = await tx.appraisalAssessment.create({
        data: {
          ...
          assessorAssignmentId: assignment.id,
          status: "DRAFT",
          revision: HEADTEACHER_SUPERVISORY_OFFICER_DRAFT_POLICY.initialRevision,
      ...
      await tx.auditLog.create({
        data: {
          ...
          userId: actor.id,
          action: OFFICER_CYCLE_CREATED_AUDIT_ACTION,
      ...
      await tx.auditLog.create({
        data: {
          tenantId: target.target.tenantId,
          userId: actor.id,
          action: OFFICER_DRAFT_CREATED_AUDIT_ACTION,
    ...
    },
    {
      ...
      maxWait: HEADTEACHER_SUPERVISORY_OFFICER_DRAFT_POLICY.transactionMaxWaitMs,
      timeout: HEADTEACHER_SUPERVISORY_OFFICER_DRAFT_POLICY.transactionTimeoutMs,
    },
  );
}

export async function createHeadteacherSupervisoryOfficerAssessmentDraft(
  input: CreateHeadteacherSupervisoryOfficerDraftInput,
): Promise<CreateHeadteacherSupervisoryOfficerDraftResult> {
  const database =
    input.database ??
    (prisma as unknown as HeadteacherSupervisoryOfficerDraftDatabase);
  const actorUserId = requireIdentifier(input.actorUserId, "actorUserId");
  const actorRole = officerRole(input.actorRoleName);
  const targetUserId = requireIdentifier(input.targetUserId, "targetUserId");
  const targetTenantId = requireIdentifier(input.targetTenantId, "targetTenantId");
  const directAssessmentKey = requireDirectAssessmentKey(input.directAssessmentKey);
  const reqId = requireIdentifier(clean(input.reqId) || randomUUID(), "reqId");
  const now = input.now ? new Date(input.now) : new Date();
  if (Number.isNaN(now.getTime())) {
    fail("HEADTEACHER_SUPERVISORY_OFFICER_DRAFT_INVALID_CURRENT_TIME", 400);
  }
  ...
  fail("HEADTEACHER_SUPERVISORY_OFFICER_DRAFT_CONCURRENT_CREATE_FAILED", 409);
}
