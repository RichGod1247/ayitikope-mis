// src/components/teacher/TeacherAssessmentSummaryClient.tsx
"use client";
...
export default function TeacherAssessmentSummaryClient() {
  ...
  const [summary, setSummary] = useState<TermSummaryResponse | null>(null);
  ...
  async function handleLoadSummary(e?: React.FormEvent) {
    ...
    if (!c || !tm || !yr) {
      setLoadError("Please fill classroom ID, term and academic year.");
      return;
    }
    ...
    try {
      ...
      try {
        ...
        setLoadState("error");
        setLoadError("Server returned an invalid response.");
        return;
      }
      ...
      const newParams = new URLSearchParams({ classroomId: c, term: tm, academicYear: yr });
      router.replace(`/teacher/assessment/summary?${newParams.toString()}`);
    } catch {
      setLoadState("error");
      setLoadError("Something went wrong while loading the summary.");
    }
  }

  useEffect(() => {
    if (initialClassroomId && initialTerm && initialAcademicYear) handleLoadSummary();
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, []);

  const classroom = summary?.classroom ?? null;
  const subjects: any[] = summary?.summary?.subjects ?? [];
  ...
  const overallGes = gesGradeFromPercentage(overallAveragePct ?? undefined);
    ...
    <main className="min-h-screen bg-slate-50">
      <div className="mx-auto max-w-5xl px-4 py-6 sm:py-8">
        <header className="mb-6 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <div>
            <h1 className="text-xl font-semibold text-slate-900 sm:text-2xl">Teacher term summary</h1>
          ...
          {overallGes && (
            <div className="rounded-lg border border-slate-200 bg-white px-3 py-2 text-xs text-slate-700 shadow-sm">
              <div className="text-[11px] text-slate-500">Overall class average</div>
              <div className="mt-1 text-lg font-semibold text-slate-900">
                {overallAveragePct != null ? `${overallAveragePct.toFixed(1)}%` : "—"}
              </div>
        ...
        <section className="mb-6 rounded-xl border border-slate-200 bg-white p-4 shadow-sm text-xs sm:text-sm">
          ...
          <form onSubmit={handleLoadSummary} className="grid gap-3 sm:grid-cols-2">
            <div className="space-y-1 sm:col-span-2">
              <label className="block text-[11px] font-medium text-slate-700">Classroom ID</label>
              <input
                className="w-full rounded border border-slate-300 px-2 py-1 text-xs font-mono"
                value={classroomId}
                onChange={(e) => setClassroomId(e.target.value)}
                placeholder="Paste a valid Classroom.id"
              />
            </div>
            ...
            <div className="sm:col-span-2 flex items-center justify-between pt-1">
              ...
              >
                {loadState === "loading" ? "Loading..." : "Load summary"}
              </button>
              <div className="text-[11px] text-slate-500">
                {summary ? `${classLabel} • ${term} • ${academicYear}` : ""}
              </div>
            </div>
          </form>
        </section>
                  ...
                  {subjects.map((s: any, idx: number) => {
                    ...
                    return (
                      <tr key={`${s.subject}-${idx}`} className={zebra}>
                        <td className="border-b border-slate-100 px-3 py-1.5 text-slate-800">{s.subject}</td>
                        ...
                        <td className="border-b border-slate-100 px-3 py-1.5 text-right text-slate-800">
                          {typeof s.averagePercentage === "number" ? `${s.averagePercentage.toFixed(1)}%` : "—"}
                        </td>
                        <td className="border-b border-slate-100 px-3 py-1.5 text-center font-semibold text-slate-900">
                          {s.grade ?? "—"}
                        </td>
                        <td className="border-b border-slate-100 px-3 py-1.5 text-slate-700">{s.remark ?? "—"}</td>
                      </tr>
                    );
                  })}
                </tbody>
