...
/>
            ...
            <SectionCard
             ...
             subtitle="See each learner's score in your assigned Mock subject(s), alongside the overall class readiness context."
              right={
                <button
                  type="button"
                  onClick={() => loadBroadsheet(sessionId)}
              ...
              <div className="space-y-4">
                ...
                {!broadsheet ? (
                  ...
                  <>
                    ...
                    <div data-teacher-mock-learner-snapshot="subject-labelled-v1" className="overflow-auto rounded-2xl border border-white/10">
                      <table className="min-w-[780px] w-full border-collapse text-left text-[12px]">
                        <thead className="bg-white/[0.05] text-[#AEB6C4]">
                          <tr>
                            <th className="border-b border-white/10 px-3 py-2">Learner</th>
                            <th className="border-b border-white/10 px-3 py-2">Your subject score(s)</th>
                            <th className="border-b border-white/10 px-3 py-2">Overall average</th>
                              ...
                              <td className="px-3 py-2 text-[#C9CDD6]">
                                <div className="space-y-1">
                                  {student.subjects.length === 0 ? (
                                    <span className="text-[#8F98A8]">No assigned subject visible</span>
                                  ) : (
                                    student.subjects.map((subjectScore) => (
                                      <div
                                        key={`${student.studentId}:${subjectScore.itemId}`}
                                        className="flex items-center justify-between gap-2"
                                      >
                                        <span className="min-w-0 truncate text-[11px] font-medium text-[#F7F4ED]">
                                          {subjectScore.subject}
                                        </span>
                                        <span className="shrink-0 text-[11px] font-semibold text-[#C9CDD6]">
                                          {subjectScore.score == null ? "—" : formatNumber(subjectScore.score)}
                                        </span>
                                      </div>
                                    ))
                                  )}
